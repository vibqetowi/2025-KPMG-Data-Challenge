...
class DataTransformer:
    def __init__(self):
        ...
        self.schemas = {
            'employees': {
                'columns': ['personnel_no', 'employee_name', 'staff_level', 'is_external'],
                'dtypes': {'personnel_no': 'int', 'employee_name': 'str', 'staff_level': 'str', 'is_external': 'bool'}
            },
            'clients': {
    ...
    def process_dictionary_data(self):
        """
        Process dictionary data to populate the dictionary table.
        """
        ...
        try:
            print(f"Processing dictionary file: {DICTIONARY_CSV}")
            ...
            for _, row in df_dict.iterrows():
                ...
                key_column = 'Key' if 'Key' in df_dict.columns else None
                desc_column = ' Description' if ' Description' in df_dict.columns else 'Description'
                
                if key_column and desc_column and pd.notna(row[key_column]) and pd.notna(row[desc_column]):
                    dict_entries.append({
            ...
            if dict_entries:
                ...
                df_dictionary = df_dictionary.drop_duplicates(subset=['key'])
                self.dfs['dictionary'] = pd.concat([self.dfs['dictionary'], df_dictionary])
                self.dfs['dictionary'] = self.dfs['dictionary'].drop_duplicates(subset=['key'], keep='last')
            
            print(f"Processed {len(dict_entries)} dictionary entries")
            
        except Exception as e:
            print(f"Error processing dictionary data: {str(e)}")
