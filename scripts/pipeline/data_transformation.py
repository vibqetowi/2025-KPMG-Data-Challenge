...
class DataTransformer:
    def __init__(self):
        ...
        # DB-generated columns that should be excluded from CSVs
                    ...
                    for col in df_staff.columns[9:]:  # Week dates start from column 10
                        try:
                            ...
                            if planned_hours > 0:
                                ...
                                # Create new row dictionary but set id to None to be replaced with NULL later
                                new_row_data = {
                                    'id': None,  # Set to None for DB to generate
    ...
    def process_timesheet_data(self):
        ...
        try:
            ...
            # Set ID column to None (will be NULL in the CSV) for database to generate
            df_time_mapped['id'] = None
    ...
    def save_transformed_data(self):
        ...
        for table_name, df in self.dfs.items():
            if not df.empty:
                ...
                if pk_columns:
                    # Only deduplicate based on non-NULL primary keys
                    # For tables where ID will be generated by DB, use other columns
                    if table_name in self.db_generated_columns and self.db_generated_columns[table_name] == pk_columns:
                        # For tables where PK is DB-generated, don't deduplicate on that column
                        non_pk_cols = [col for col in df.columns if col not in self.db_generated_columns[table_name]]
                        if non_pk_cols:
                            df = df.drop_duplicates(subset=non_pk_cols, keep='last')
                    else:
                        df = df.drop_duplicates(subset=pk_columns, keep='last')
                    
                    self.dfs[table_name] = df
                
                # Save the DataFrame with NULL values for DB-generated columns
                output_file = OUTPUT_DIR / f"{table_name}.csv"
                df.to_csv(output_file, index=False)
                
                # Report how many DB-generated columns were set to NULL
                db_generated = self.db_generated_columns.get(table_name, [])
                if db_generated:
                    print(f"Saved {len(df)} rows to {output_file} (DB will generate: {db_generated})")
                else:
                    print(f"Saved {len(df)} rows to {output_file}")
    
    def transform_all_data(self):
        """
        Process all data sources and save transformed CSVs.
        """
