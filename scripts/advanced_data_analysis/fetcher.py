...
class DataFetcher:
    ...
    def __init__(self, source='db', csv_base_path=None, timeout=5):
        ...
        if csv_base_path is None:
            self.csv_base_path = Path('/Users/notAdmin/Dev/2025 KPMG Data Challenge/csv-dump')
        else:
        ...
        if self.source == 'db':
            ...
            try:
                ...
                print("Database connection initialized. Will test actual connection when fetching data.")
    ...
    def _test_db_connection(self):
        ...
        connection_thread = threading.Thread(target=lambda: setattr(self, '_connection_result', test_connection()))
    ...
    def fetch_data(self, tables=None):
        ...
        if tables is None:
            tables = ['timesheets', 'employees', 'clients', 'engagements', 'phases', 'practices']
    ...
    def fetch_table(self, table_name):
        ...
        if self.source == 'db':
            try:
                ...
                print(f"Successfully loaded {len(df)} records from {table_name} table in database.")
                return df
            ...
            try:
                ...
                for encoding in ['utf-8-sig', 'latin1', 'cp1252']:
                    try:
                        ...
                        print(f"Successfully loaded {len(df)} records from {table_name} CSV using {encoding} encoding.")
                        ...
                        if encoding == 'cp1252':  # Last encoding to try
                            print(f"Error loading {table_name} with encoding {encoding}: {e}")
                        continue
    ...
    def _transform_raw_csv_to_table(self, df, table_name):
        ...
        if table_name == 'timesheets':
            ...
            if 'Personnel No.' in df.columns and 'Employee Name' in df.columns:
                df = df[['Personnel No.', 'Employee Name', 'Staff Level']].drop_duplicates()
            ...
            try:
                ...
                for encoding in ['utf-8-sig', 'latin1', 'cp1252']:
                    try:
                        ...
                        print(f"Successfully loaded {len(df_time)} records from CSV file using {encoding} encoding.")
                        break
                    except:
                        continue
